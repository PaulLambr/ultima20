...
while len(deck) >= 2:  # Ensure there are enough cards for a round
    ...
    if player1_value > player2_value:
        ...
        if player1_prisoner and card_value(player1_prisoner) > card_value(
            player1_war_card
        ):
            use_prisoner = input("Player 1, use your prisoner? (y/n): ").lower() == "y"
            if use_prisoner:
                print(
                    f"Player 1 uses their prisoner: {player1_prisoner[0]} of {player1_prisoner[1]}"
                )
                player1_war_card = player1_prisoner
                player1_prisoner = None

        if player2_prisoner and card_value(player2_prisoner) > card_value(
            player2_war_card
        ):
            use_prisoner = input("Player 2, use your prisoner? (y/n): ").lower() == "y"
            if use_prisoner:
                print(
                    f"Player 2 uses their prisoner: {player2_prisoner[0]} of {player2_prisoner[1]}"
                )
